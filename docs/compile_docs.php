...
function parseMarkdownFile($section, $filePath) {
    ...
    foreach ($lines as $line) {
        ...
        // Manejar Alertas / Bloques citados (> [!NOTE], > [!WARNING])
        if (str_starts_with($trimmed, '>')) {
        ...
        if (preg_match('/^#\s+(.+)$/', $trimmed, $matches)) {
            // Ignorar el título principal (#) ya que usamos la portada y el título de Parte I/II
            continue;
        }
        if (preg_match('/^##\s+(.+)$/', $trimmed, $matches)) {
        ...
        // Marcadores de Captura de Pantalla
    ...
    // Si el archivo termina y estábamos en una alerta
    if ($inAlert) {
        writeAlertBox($section, $alertType, $alertText);
    }
}
...
function addFormattedText($textRun, $line) {
    // Procesar negritas con regex **texto**
